ConVar g_hCvarMaxWarns, g_hCvarMaxPunishment, g_hCvarBanLength, g_hCvarPunishment, g_hCvarSlapDamage, g_hCvarPrintToAdmins,
		...
		g_hCvarLogQuery, g_hCvarWarnLength, g_hCvarWarnTime, g_hCvarMaxScore;
...
public void InitializeConVars()
{
	...
	(g_hCvarMaxWarns = CreateConVar("sm_warns_maxwarns", "3", "Max warnings before punishment", _, true, 0.0, true, 10.0)).AddChangeHook(ChangeCvar_MaxWarns);
	...
	(g_hCvarLogQuery = CreateConVar("sm_warns_enable_querylog", "0", "Logging query to DB: 0 - disabled, 1 - enabled", _, true, 0.0, true, 1.0)).AddChangeHook(ChangeCvar_LogQuery);
	...
	(g_hCvarWarnTime = CreateConVar("sm_warns_warntime_type", "1", "Take duration from config if set 1 or from cvar ('sm_warns_warnlength') for all if set 0: 0 - cvar, 1 - config.")).AddChangeHook(ChangeCvar_WarnTime);
	(g_hCvarMaxScore = CreateConVar("sm_warns_maxscore", "50", "Max score (points) before punishment", _, true, 0.0)).AddChangeHook(ChangeCvar_MaxScore);
